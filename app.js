...
function food(c){
 ...
 <div class="weekly-menu">${days.map((d,i)=>{const x=slots[i]||{};return `<div class="menu-day"><div class="menu-day-head"><b>${d}</b>${x.tupper?'<span class="pill">Tupper</span>':''}</div><div class="meal-line"><span>Comida</span>${menuRecipeTitle(x.lunch,'menu-meal-title')}${state.menu?.manualSlots?.[`${i}:lunch`]?'<span class="pill menu-manual-pill">Elegida</span>':''}<button class="icon-button" onclick="openMenuRecipePicker(${i},'lunch')" title="Elegir receta">☰</button><button class="icon-button" onclick="changeMeal(${i},'lunch')" title="Cambiar sugerencia">↻</button></div><div class="meal-line"><span>Cena</span>${menuRecipeTitle(x.dinner,'menu-meal-title')}${state.menu?.manualSlots?.[`${i}:dinner`]?'<span class="pill menu-manual-pill">Elegida</span>':''}<button class="icon-button" onclick="openMenuRecipePicker(${i},'dinner')" title="Elegir receta">☰</button><button class="icon-button" onclick="changeMeal(${i},'dinner')" title="Cambiar sugerencia">↻</button></div><div class="meal-line breakfast"><span>Desayuno</span>${menuRecipeTitle(x.breakfast,'menu-meal-title')}</div><div class="meal-line snack"><span>Merienda</span>${x.snack?menuRecipeTitle(x.snack,'menu-meal-title'):''}</div></div>`}).join('')||'<div class="empty-state">Genera una propuesta para empezar.</div>'}</div>
