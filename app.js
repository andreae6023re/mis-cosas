...
function food(c){
 ...
 <section class="food-panel"><div class="panel-heading"><div><h3>Preparar esta semana</h3><span>${pendingPrep} pendientes</span></div><button class="secondary small" onclick="openModal('Nueva preparación',preparationForm())">+ Añadir</button></div><div class="prep-list">${prep.slice(0,6).map(x=>`<div class="prep-row ${state.prepDone[x.id]?'done':''}"><button class="check-mini" onclick="event.stopPropagation();togglePrep('${x.id}')" aria-label="Marcar preparación">${state.prepDone[x.id]?'✓':''}</button><button class="prep-content" onclick="viewPreparation('${x.id}')"><b>${esc(x.name)}</b><small>${esc(x.quantity||'')} · ${x.freezable?'Congelable':'Para consumir'}</small></button></div>`).join('')||'<div class="muted">Todavía no tienes preparaciones.</div>'}</div>${prep.length>6?`<button class="text-button" onclick="openModal('Preparaciones',preparationsListForm())">Ver todas →</button>`:''}</section></aside></div>
