...
function save(){Object.keys(state).forEach(k=>{if(Array.isArray(state[k])||k==='settings'||k==='menu')localStorage.setItem(KEY+k,JSON.stringify(state[k]))})}
...
function food(c){
 ...
 const slots=menu?.days||{};
 const foodPrefs=state.settings.food||{};
 c.innerHTML=`<div class="food-page">
  ...
  <section class="food-preferences"><button class="pref-chip ${state.settings.food?.useInventory!==false?'active':''}" onclick="toggleFoodPref('useInventory')">Aprovechar lo que tengo</button><button class="pref-chip ${state.settings.food?.quickMeals!==false?'active':''}" onclick="toggleFoodPref('quickMeals')">Comidas rápidas</button><button class="pref-chip ${state.settings.food?.vegetables!==false?'active':''}" onclick="toggleFoodPref('vegetables')">Más verduras</button><button class="pref-chip ${state.settings.food?.twoTuppers!==false?'active':''}" onclick="toggleFoodPref('twoTuppers')">2 tuppers</button><button class="pref-chip ${state.settings.food?.sweet?'active':''}" onclick="toggleFoodPref('sweet')">Incluir dulce</button><button class="pref-chip ${state.settings.food?.bread?'active':''}" onclick="toggleFoodPref('bread')">Hacer pan</button></section>
  ...
  <div class="food-layout">
   <section class="food-panel menu-panel"><div class="panel-heading"><div><h3>Menú semanal</h3><span>${menu?.status==='accepted'?'Menú aceptado':'Propuesta pendiente de revisar'}</span></div>${menu?`<div class="actions"><button class="secondary small" onclick="generateMenu()">Regenerar</button><button class="primary small" onclick="acceptMenu()">Aceptar menú</button></div>`:''}</div>
    <div class="weekly-menu">${days.map((d,i)=>{const x=slots[i]||{};return `<div class="menu-day"><div class="menu-day-head"><b>${d}</b>${x.tupper?'<span class="pill">Tupper</span>':''}${i===4?'<span class="pill">Comida divertida</span>':''}</div><div class="meal-line"><span>Comida</span><strong>${esc(x.lunch||'Sin planificar')}</strong><button class="icon-button" onclick="changeMeal(${i},'lunch')">↻</button></div><div class="meal-line"><span>Cena</span><strong>${esc(x.dinner||'Sin planificar')}</strong><button class="icon-button" onclick="changeMeal(${i},'dinner')">↻</button></div><div class="meal-line breakfast"><span>Desayuno</span><strong>${esc(x.breakfast||'Café con leche + tostada')}</strong></div><div class="meal-line snack"><span>Merienda</span><strong>${esc(x.snack||'Fruta o yogur vegetal')}</strong></div></div>`}).join('')||'<div class="empty-state">Genera una propuesta para empezar.</div>'}</div>
   </section>
   <aside class="food-side"><section class="food-panel"><div class="panel-heading"><div><h3>Inventario</h3><span>Lo que tienes ahora</span></div><button class="secondary small" onclick="openModal('Nuevo producto',inventoryForm())">+ Añadir</button></div><div class="inventory-mini"><div><b>Congelador</b><span>${counts.freezer}</span></div><div><b>Despensa</b><span>${counts.pantry}</span></div><div><b>Frescos</b><span>${counts.fresh}</span></div></div><button class="text-button" onclick="openModal('Inventario',inventoryListForm())">Ver y gestionar inventario →</button></section>
    <section class="food-panel"><div class="panel-heading"><div><h3>Preparar esta semana</h3><span>Preparaciones que puedes adelantar</span></div><button class="secondary small" onclick="openModal('Nueva preparación',preparationForm())">+ Añadir</button></div><div class="prep-list">${prep.slice(0,5).map(x=>`<div class="prep-row"><span class="check-mini"></span><div><b>${esc(x.name)}</b><small>${esc(x.quantity||'')} · ${x.freezable?'Congelable':'Para consumir'}</small></div></div>`).join('')||'<div class="muted">Todavía no tienes preparaciones.</div>'}</div></section></aside>
  </div>
  <section class="food-panel recipes-panel"><div class="panel-heading"><div><h3>Mis recetas</h3><span>Comidas, cenas, dulces, pan y preparaciones</span></div><button class="primary small" onclick="openModal('Añadir receta',recipeForm())">+ Añadir receta</button></div><div class="recipe-grid">${state.recipes.map(r=>`<article class="recipe-card"><div class="recipe-card-top"><span class="pill">${esc(r.type)}</span>${r.favorite?'<span class="favorite-mark">★</span>':''}</div><h4>${esc(r.name)}</h4><p>${esc(r.description||'Sin descripción')}</p><div class="recipe-meta"><span>${r.servings||1} raciones</span><span>${esc(r.time||'Tiempo no indicado')}</span>${r.freezable?'<span>Congelable</span>':''}</div><div class="actions"><button class="secondary small" onclick="viewRecipe('${r.id}')">Ver</button><button class="secondary small" onclick="editRecipe('${r.id}')">Editar</button></div></article>`).join('')||'<div class="empty-state">Todavía no tienes recetas. Añade la primera.</div>'}</div></section>
  <section class="food-panel shopping-panel"><div class="panel-heading"><div><h3>Lista de la compra</h3><span>${shoppingList().length} productos pendientes</span></div><button class="secondary small" onclick="openModal('Lista de la compra',shoppingForm())">Ver lista</button></div><div class="shopping-preview">${shoppingList().slice(0,6).map(x=>`<span class="shopping-item">${esc(x)}</span>`).join('')||'<span class="muted">Se calculará al aceptar un menú.</span>'}</div></section>
 </div>`;
}
...
function generateMenu(){const candidates=state.recipes.filter(r=>r.type==='Comidas'||r.type==='Cenas');const preferred=state.recipes.filter(r=>r.favorite&&(r.type==='Comidas'||r.type==='Cenas'));const pool=[...preferred,...candidates.filter(r=>!r.favorite)];const unique=[];pool.forEach(r=>{if(!unique.some(x=>x.id===r.id))unique.push(r)});if(!unique.length){alert('Añade algunas recetas de Comidas o Cenas antes de generar el menú.');return}const f=state.settings.food||{};const invNames=state.inventory.map(x=>String(x.name||'').toLowerCase());const score=r=>{let score=0;(r.ingredients||[]).forEach(i=>{const n=String(i.ingredient||'').toLowerCase();if(n&&invNames.some(x=>x.includes(n)||n.includes(x)))score+=2});if(r.favorite)score+=1;return score};unique.sort((a,b)=>score(b)-score(a));const pick=(offset)=>unique[offset%unique.length].name;const daysObj={};for(let i=0;i<7;i++){daysObj[i]={lunch:pick(i),dinner:pick((i+2)%unique.length),breakfast:i<5?(f.breakfast||'Café con leche + tostada con aceite y tomate'):'Desayuno especial sencillo',snack:f.snacks!==false?(i===4&&f.sweet?'Dulce saludable':'Fruta o yogur vegetal'):'',tupper:f.twoTuppers!==false&&[1,3].includes(i)}}state.menu={week:monthKey(),status:'proposal',days:daysObj,generatedAt:new Date().toISOString()};save();render()}
function acceptMenu(){if(!state.menu)return;state.menu.status='accepted';save();render()}
function changeMeal(day,slot){if(!state.menu)return;const list=state.recipes.filter(r=>r.type==='Comidas'||r.type==='Cenas');if(!list.length){alert('Añade alguna receta primero.');return}const current=state.menu.days[day]?.[slot];const idx=Math.max(0,list.findIndex(r=>r.name===current));state.menu.days[day][slot]=list[(idx+1)%list.length].name;state.menu.status='proposal';save();render()}
function shoppingList(){if(!state.menu||state.menu.status!=='accepted')return [];const needed=[];Object.values(state.menu.days||{}).forEach(day=>{[day.lunch,day.dinner].forEach(name=>{const r=state.recipes.find(x=>x.name===name);(r?.ingredients||[]).forEach(i=>{const ing=String(i.ingredient||'').toLowerCase();const has=ing&&state.inventory.some(x=>{const n=String(x.name||'').toLowerCase();return n.includes(ing)||ing.includes(n)});if(ing&&!has)needed.push(`${i.ingredient}${i.quantity?' · '+i.quantity+' '+i.unit:''}`)})})});return [...new Set(needed)]}
function shoppingForm(){const list=shoppingList();return `<div class="shopping-list-modal">${list.map((x,i)=>`<label class="checkline"><input type="checkbox" id="shop-${i}"> ${esc(x)}</label>`).join('')||'<p class="muted">No hay compras pendientes.</p>'}</div>`}
...
function notificationSettingsForm(){const n=state.settings.notifications||{};return `<div class="form">${[['tasks','Tareas'],['calendar','Calendario'],['expenses','Gastos y presupuesto'],['food','Comidas y preparación']].map(([k,l])=>`<label class="checkline"><input id="notif-${k}" type="checkbox" ${n[k]!==false?'checked':''}> ${l}</label>`).join('')}<button class="primary" onclick="saveNotificationSettings()">Guardar</button></div>`}
