...
function settings(c){
 const recurring=state.transactions.filter(t=>t.recurring&&!t.recurringFrom);
 const activeCats=Object.entries(expenseCategories).flatMap(([g,a])=>a.map(x=>({group:g,name:x})));
 c.innerHTML=`<div class="settings-page">
 <div class="settings-intro"><span class="eyebrow">Configuración</span><h2>Todo a tu medida</h2><p>Configura aquí las partes de Gastos que quieres mantener estables mes a mes.</p></div>
 <div class="settings-grid">
  <section class="card settings-card"><div class="setting-head"><div><h3>Cuentas</h3><span class="muted">Nombre, saldo inicial y estado.</span></div><button class="secondary small" onclick="openModal('Configurar cuentas',accountsSettingsForm())">Editar</button></div><div class="settings-list">${state.accounts.map(a=>`<div class="setting-row"><div><b>${esc(a.name)}</b><span>${a.active===false?'Inactiva':'Activa'} · saldo inicial</span></div><strong>${money(a.startingBalance||0)}</strong></div>`).join('')}</div></section>
  <section class="card settings-card"><div class="setting-head"><div><h3>Categorías</h3><span class="muted">Organiza tus gastos e ingresos.</span></div><button class="secondary small" onclick="openModal('Gestionar categorías',categoriesSettingsForm())">Editar</button></div><div class="settings-list">${Object.entries(expenseCategories).map(([g,a])=>`<div class="setting-row"><div><b>${esc(g)}</b><span>${a.length} categorías</span></div><span class="pill">${a.slice(0,2).map(esc).join(' · ')}${a.length>2?'…':''}</span></div>`).join('')}</div></section>
  <section class="card settings-card"><div class="setting-head"><div><h3>Presupuestos</h3><span class="muted">Límites mensuales por categoría.</span></div><button class="secondary small" onclick="go('expenses')">Gestionar</button></div><p class="muted">Los presupuestos se guardan por mes para que puedas cambiar, por ejemplo, el límite de Comida sin afectar a meses anteriores.</p></section>
  <section class="card settings-card"><div class="setting-head"><div><h3>Movimientos recurrentes</h3><span class="muted">Gastos, ingresos y transferencias automáticas.</span></div><button class="secondary small" onclick="go('expenses')">Ver gastos</button></div><div class="settings-list">${recurring.slice(0,4).map(t=>`<div class="setting-row"><div><b>${esc(t.concept||'Movimiento')}</b><span>${t.type==='transfer'?'Transferencia':'Movimiento'} · día ${t.recurringDay||'—'}</span></div><button class="secondary small" onclick="${t.type==='transfer'?`editTransfer('${t.id}')`:`editExpense('${t.id}')`}">Editar</button></div>`).join('')||'<span class="muted">No hay movimientos recurrentes todavía.</span>'}</div></section>
  <section class="card settings-card"><div class="setting-head"><div><h3>Copias de seguridad</h3><span class="muted">Guarda tus datos o recupéralos.</span></div></div><div class="data-actions"><button class="secondary" onclick="exportData()">Exportar mis datos</button><button class="secondary" onclick="document.querySelector('#importDataInput').click()">Importar copia</button><input id="importDataInput" type="file" accept="application/json" hidden onchange="importData(this)"></div><p class="muted">La copia incluye tareas, gastos, cuentas, presupuestos, calendario, hábitos, recetas y configuración.</p></section>
  <section class="card settings-card"><div class="setting-head"><div><h3>Estado de la aplicación</h3><span class="muted">Datos guardados en este dispositivo.</span></div><span class="status-dot">● Local</span></div><p class="muted">La aplicación funciona sin servidor: tus datos se mantienen en el navegador de este dispositivo.</p></section>
 </div></div>`;
}
...
function exportData(){
 const data={version:8,exportedAt:new Date().toISOString(),tasks:state.tasks,expenses:state.expenses,transactions:state.transactions,accounts:state.accounts,budgets:state.budgets,events:state.events,habits:state.habits,recipes:state.recipes,expenseCategories};
...
async function importData(input){const file=input.files?.[0];if(!file)return;try{const data=JSON.parse(await file.text());if(!data||typeof data!=='object'||!Array.isArray(data.transactions)||!Array.isArray(data.accounts))throw new Error('Formato no válido');if(!confirm('Esto sustituirá los datos actuales por los de la copia. ¿Continuar?')){input.value='';return}['tasks','expenses','transactions','accounts','budgets','events','habits','recipes'].forEach(k=>{if(Array.isArray(data[k]))state[k]=data[k]});if(data.expenseCategories&&typeof data.expenseCategories==='object')expenseCategories=data.expenseCategories;save();saveCategories();input.value='';render();alert('Copia restaurada correctamente.')}catch(e){input.value='';alert('No se ha podido importar la copia. Comprueba que sea un archivo de Mis cosas.')}}
