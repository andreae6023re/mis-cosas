...
function normalizeRecipes(){state.recipes=state.recipes.map(r=>{const types=recipeTypes(r);const seasons=Array.isArray(r.seasons)?r.seasons:[];return {...r,types,seasons}})}
...
function food(c){
 ...
 <section class="food-panel"><div class="panel-heading"><div><h3>Preparar esta semana</h3><span>${pendingPrep} pendientes</span></div><button class="secondary small" onclick="document.querySelector('#prepImportInput').click()">Importar</button><input id="prepImportInput" type="file" accept=".json,.csv,application/json,text/csv" hidden onchange="importPreparations(this)"><button class="secondary small" onclick="openModal('Nueva preparación',preparationForm())">+ Añadir</button></div><div class="prep-list">${prepOrder.slice(0,6).map(x=>`<button class="prep-row ${state.prepDone[x.id]?'done':''}" onclick="togglePrep('${x.id}')"><span class="check-mini">${state.prepDone[x.id]?'✓':''}</span><div><b>${esc(x.name)}</b><small>${esc(x.quantity||'')} · ${x.freezable?'Congelable':'Para consumir'}</small></div></button>`).join('')||'<div class="muted">Todavía no tienes preparaciones.</div>'}</div>${preparationsForMenu().length?`<div class="menu-preparations"><small class="muted">Necesarias para este menú</small>${preparationsForMenu().map(x=>`<div class="prep-needed ${state.prepDone[x.id]?'done':''}"><span>${state.prepDone[x.id]?'✓':'○'}</span><b>${esc(x.name)}</b></div>`).join('')}</div>`:''}${prep.length>6?`<button class="text-button" onclick="openModal('Preparaciones',preparationsListForm())">Ver todas →</button>`:''}</section></aside></div>
...
function preparationForm(item=null){return `<div class="form"><label>Preparación<input id="fPrepName" value="${esc(item?.name||'')}" placeholder="Ej. sofrito casero"></label><label>Cantidad / rendimiento<input id="fPrepQty" value="${esc(item?.quantity||'')}" placeholder="Ej. 4 usos o 500 g"></label><label>Uso previsto<input id="fPrepUse" value="${esc(item?.use||'')}" placeholder="Ej. arroz, pasta, guisos"></label><label>Ingredientes <small>uno por línea: ingrediente | cantidad | unidad</small><textarea id="fPrepIngredients" placeholder="Cebolla | 250 | g
Aceite de oliva | 20 | ml">${esc(item?.ingredientsText||'')}</textarea></label><label>Preparación<textarea id="fPrepSteps" placeholder="Pasos de elaboración">${esc(item?.steps||'')}</textarea></label><label>Descripción<textarea id="fPrepDesc" placeholder="Para qué sirve esta preparación">${esc(item?.description||'')}</textarea></label><label class="checkline"><input id="fPrepFreeze" type="checkbox" ${item?.freezable?'checked':''}> Se puede congelar</label><button class="primary" onclick="savePreparation('${item?.id||''}')">${item?'Guardar cambios':'Guardar preparación'}</button>${item?`<button class="danger-button" onclick="deletePreparation('${item.id}')">Eliminar</button>`:''}</div>`}
function savePreparation(id=''){const name=document.querySelector('#fPrepName')?.value.trim();if(!name)return;let x=id?state.preparations.find(i=>i.id===id):null;if(!x){x={id:crypto.randomUUID()};state.preparations.push(x)}x.name=name;x.quantity=document.querySelector('#fPrepQty')?.value.trim()||'';x.use=document.querySelector('#fPrepUse')?.value.trim()||'';x.ingredientsText=document.querySelector('#fPrepIngredients')?.value.trim()||'';x.ingredients=parseIngredients(x.ingredientsText);x.steps=document.querySelector('#fPrepSteps')?.value.trim()||'';x.description=document.querySelector('#fPrepDesc')?.value.trim()||'';x.freezable=!!document.querySelector('#fPrepFreeze')?.checked;localCacheFromState();closeModal();render();void syncCloudField('preparations',state.preparations)}
...
async function importPreparations(input){const file=input.files?.[0];if(!file)return;try{const text=await file.text();let incoming;if(file.name.toLowerCase().endsWith('.csv')){const lines=text.split(/\r?\n/).filter(Boolean);if(!lines.length)throw new Error('vacío');const headers=lines.shift().split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/).map(x=>x.trim().replace(/^"|"$/g,''));incoming=lines.map(line=>{const cols=line.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/).map(x=>x.trim().replace(/^"|"$/g,''));const o={};headers.forEach((h,i)=>o[h]=cols[i]??'');return o})}else incoming=JSON.parse(text);if(!Array.isArray(incoming))throw new Error('formato');const valid=incoming.map(x=>({...x,id:x.id||crypto.randomUUID(),name:String(x.name||'').trim(),quantity:String(x.quantity||'').trim(),use:String(x.use||'').trim(),freezable:!!x.freezable,ingredientsText:String(x.ingredientsText||'').trim(),ingredients:Array.isArray(x.ingredients)?x.ingredients:parseIngredients(String(x.ingredientsText||'')),steps:String(x.steps||'').trim(),description:String(x.description||'').trim()})).filter(x=>x.name);let added=0,updated=0;const byId=new Map(state.preparations.map(x=>[String(x.id),x]));for(const p of valid){const existing=byId.get(String(p.id));if(existing){Object.assign(existing,p);updated++}else{state.preparations.push(p);added++}}localCacheFromState();render();if(supabaseClient&&currentUser)await syncCloudField('preparations',state.preparations);input.value='';alert(`Preparaciones importadas: ${added} nuevas y ${updated} actualizadas.`)}catch(e){input.value='';console.error(e);alert('No se han podido importar las preparaciones. Comprueba que sea un JSON o CSV válido.')}}

async function saveRecipe(id=''){const name=document.querySelector('#fRecipeName')?.value.trim();if(!name)return;let r=id?state.recipes.find(x=>x.id===id):null;if(!r){r={id:crypto.randomUUID()};state.recipes.push(r)}const types=[...document.querySelectorAll('.fRecipeType:checked')].map(x=>x.value);const seasons=[...document.querySelectorAll('.fRecipeSeason:checked')].map(x=>x.value);r.name=name;r.types=types.length?types:['Comida'];r.type=r.types[0];r.seasons=seasons;r.servings=Number(document.querySelector('#fRecipeServings').value)||1;r.time=document.querySelector('#fRecipeTime').value.trim();r.ingredients=parseIngredients(document.querySelector('#fRecipeIngredients').value);r.ingredientsText=document.querySelector('#fRecipeIngredients').value;r.steps=document.querySelector('#fRecipeSteps').value;r.description=document.querySelector('#fRecipeDesc').value;r.pairs=document.querySelector('#fRecipePairs').value;r.preparationIds=[...document.querySelectorAll('.fRecipePrep:checked')].map(x=>x.value);r.favorite=document.querySelector('#fRecipeFav').checked;r.freezable=document.querySelector('#fRecipeFreezable').checked;r.funRecipe=document.querySelector('#fRecipeFun')?.checked||false;localCacheFromState();closeModal();render();if(supabaseClient&&currentUser){await syncCloudField('recipes',state.recipes)}}
...
function openMenuRecipePicker(day,slot){
 ...
 const all=type==='Cena'?dinnerMenuPool():menuRecipePool([type]);
...
function aggregateMenuIngredients(){
 ...
 const addIngredient=(i)=>{const raw=String(i?.ingredient||'').trim();if(!raw)return;const key=normalizeIngredient(raw);const q=parseInventoryQuantity(`${i.quantity||''} ${i.unit||''}`);const existing=map.get(key);if(!existing){map.set(key,{ingredient:raw,quantity:i.quantity||'',unit:i.unit||'',numeric:q?.value??null,count:1});}else if(existing.numeric!=null&&q?.value!=null&&existing.unit===i.unit){existing.numeric+=q.value;existing.quantity=String(existing.numeric);existing.count++}else{existing.count++}};
 const neededPrepIds=new Set();
 Object.values(state.menu.days||{}).forEach(day=>{['lunch','dinner','breakfast','snack'].forEach(slot=>{const r=state.recipes.find(x=>x.name===day?.[slot]);if(!r)return;(r.ingredients||parseIngredients(r.ingredientsText||'')).forEach(addIngredient);preparationIdsForRecipe(r).forEach(id=>neededPrepIds.add(String(id)));});});
 neededPrepIds.forEach(id=>{const p=state.preparations.find(x=>String(x.id)===id);if(!p||state.prepDone[p.id])return;(p.ingredients||parseIngredients(p.ingredientsText||'')).forEach(addIngredient);});
 return [...map.values()];
}
