...
async function toggleHabit(id){const h=state.habits.find(x=>x.id===id);if(!h)return;const k=todayKey();h.completed=Array.isArray(h.completed)?h.completed:[];const i=h.completed.indexOf(k);if(i>=0)h.completed.splice(i,1);else h.completed.push(k);localCacheFromState();render();if(supabaseClient&&currentUser){await syncCloudField('habits',state.habits)}}
