...
function settings(c){
 ...
 c.innerHTML=`<div class="settings-page"><div class="settings-intro"><span class="eyebrow">Configuración</span><h2>Todo a tu medida</h2><p>La siguiente parte del proyecto deja Gastos listo para usar de verdad.</p></div>
 <div class="settings-grid"><section class="card"><div class="setting-head"><div><h3>Cuentas</h3><span class="muted">Saldos iniciales y cuentas activas.</span></div><button class="secondary small" onclick="openModal('Configurar cuentas',accountsForm())">Editar</button></div><div class="settings-list">${state.accounts.map(a=>`<div class="setting-row"><span>${esc(a.name)}</span><b>${money(a.startingBalance||0)}</b></div>`).join('')}</div></section>
 <section class="card"><div class="setting-head"><div><h3>Recurrentes</h3><span class="muted">Gastos, ingresos y transferencias mensuales.</span></div></div><div class="settings-list">${recurring.map(t=>`<div class="setting-row"><div><b>${esc(t.concept||'Movimiento')}</b><span>${t.type==='transfer'?'Transferencia':'Movimiento'} · día ${t.recurringDay||'—'}</span></div><button class="secondary small" onclick="${t.type==='transfer'?`editTransfer('${t.id}')`:`editExpense('${t.id}')`}">Editar</button></div>`).join('')||'<span class="muted">No hay movimientos recurrentes.</span>'}</div></section>
 <section class="card"><div class="setting-head"><div><h3>Presupuestos</h3><span class="muted">Se configuran desde cada mes en Gastos.</span></div><button class="secondary small" onclick="go('expenses')">Ir a Gastos</button></div><p class="muted">Puedes definir un límite para Comida, Transporte, Suscripciones y cualquier otra categoría.</p></section>
